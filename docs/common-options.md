...
| `--data-dir <dir>`                        | —                | Directory for the CSV leaf used by `--provider csv` or `CSV:` symbols.                                                |
| `--csv-alignment utc-24x7`                | `unknown`        | Host assertion that CSV bars use the fixed UTC 24×7 grid; enables exact planning.                                     |
| `--csv-week-anchor <YYYY-MM-DD\|seconds>` | —                | Opening timestamp on a CSV weekly grid; requires UTC alignment.                                                       |
| `--csv-calendar <file.json>`              | —                | Strict exchange-session calendar; conflicts with CSV alignment/week-anchor.                                           |
| `--csv-complete-record`                   | off              | Assert that missing bars inside each exact CSV file's authenticated span mean no trades.                              |

All `--csv-*` claims require `--data-dir` but do not require `--provider csv`,
so they configure `CSV:` instruments in mixed routing. Complete-record also
requires an explicit alignment/calendar assertion. See
[CSV data files](./csv-data.md#exact-acquisition-and-bar-magnifier) for the
strict calendar schema, risks, and fail-closed behavior.
...
- complete symbol/window/session coverage, proven either from bars (the
  backward-compatible default) or an authenticated CSV complete-record span;
...
| ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Shipped self-contained `pinerun` binary and this root checkout:** `@heyphat/piner` **0.11.1** | Contract-capable with traversal enabled. Pinerun authenticates exact magnifier/static-security data first; piner alone reports whether magnification was active and supplies target, coverage, counters, and fill timestamps.        |
| **Older or contract-incomplete piner**                                                          | Fails metadata capability preflight with permanent code `piner-bar-magnifier-capability-unavailable`. No exact dataset is prepared. An explicit/source-effective false setting still follows ordinary chart OHLC.                    |
| **Future contract-capable piner with traversal disabled**                                       | May map, acquire, validate, and inject exact data, then must authoritatively report `requested: true`, `active: false`; fills remain chart OHLC. This is the only state in which “exact data prepared, requested/inactive” is valid. |
...
| **CSV provider**                                          | CLI exact filenames are discovered with `timeframes: 'arbitrary'`; explicit UTC or strict exchange-calendar evidence enables exact planning | Defaults remain `unknown` alignment and bars-only coverage. `--csv-alignment`/`--csv-calendar` are host assertions. `--csv-complete-record` authenticates full-file record spans and allows internal no-trade holes, but requires explicit alignment and never accepts bare fallback files. File fingerprints, calendar periods, semantics, and spans bind source/proof identity. |
...
presence of injected data. The shipped 0.11.1 runtime can report active
traversal after exact preparation. A future compatible but traversal-disabled
runtime may produce the requested/inactive line; an active line always requires
piner itself to report active traversal.
