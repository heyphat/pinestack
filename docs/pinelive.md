...
authoritative chart finals. A lower-bars every-update source instead polls
closed child history at `live.throttleMs`, validates an exact contiguous child
grid, emits one forming chart revision per newly closed child, and commits only
a separately polled chart final that exactly matches the child aggregation.
Pinelive does not invoke provider `liveBars()` in either path. An optional nested
