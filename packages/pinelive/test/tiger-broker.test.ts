...
test('TigerBroker production safety guard fails closed and is rechecked before transmission', async () => {
  const transport = new FixtureTradingTransport();
  let safe = true;
  const broker = new TigerBroker({
    transport,
  ...
  broker.setExecutionSafetyGuard({
    assertExecutionSafe() {
      if (!safe) throw new Error('claim lost');
    },
  });
  safe = false;
  await expect(broker.submit(order)).rejects.toMatchObject({
  ...
  safe = true;
  await expect(broker.submit(order)).resolves.toMatchObject({ status: 'filled' });
  expect(transport.submits).toBe(1);

  broker.clearExecutionSafetyGuard();
  await expect(broker.submit({ ...order, clientId: 'guard-cleared' })).rejects.toMatchObject({
    code: 'precondition',
  });
  expect(transport.submits).toBe(1);
});

test('TigerBroker refuses synchronized execution without authoritative exact lookup', async () => {
  const transport = new FixtureTradingTransport();
  let synchronizationCalls = 0;
  transport.synchronizeAccount = async (_accountId, symbol) => {
    ...
    return {
      snapshot: {
        ...
        account: { id: 'demo', currency: 'USD', balance: 100_000, equity: 100_000 },
  ...
  const broker = new TigerBroker({ transport, armed: true, requireExecutionSafety: true });

  const result = await broker.synchronizeAccount('MGCZ24');
  expect(result).toMatchObject({ status: 'blocked' });
  if (result.status !== 'blocked') throw new Error('transport unexpectedly synchronized');
  expect(result.reasons.join(' ')).toContain('authoritative exact order lookup');
  expect(synchronizationCalls).toBe(0);
});

test('TigerBroker closes a failed synchronization session before reporting blocked', async () => {
  const transport = new FixtureTradingTransport();
  transport.lookupOrderExact = async () => ({ status: 'not-found' });
  let closes = 0;
  transport.synchronizeAccount = async (_accountId, symbol) => ({
    snapshot: {
      synchronizationVersion: 1,
  ...
  expect(result).toMatchObject({ status: 'blocked' });
  if (result.status !== 'blocked') throw new Error('transport unexpectedly synchronized');
  expect(result.reasons.join(' ')).toContain('synchronization cleanup failed');
